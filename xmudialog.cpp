...
double XmuDialog::calculateMass(const QString &formula, QString &disp)
{
    ...
    QRegExp isotopeRegExp("(\\[[1-9][0-9]{0,2}\\])?"
                          "([A-Z][a-z]{0,2})([0-9]+)?");
    ...
    {
        if (formula.indexOf(isotopeRegExp, pos) != pos) return Invalid;
        pos += isotopeRegExp.matchedLength();
        if (isotopeRegExp.captureCount() != 3) return Invalid;
        symbol = isotopeRegExp.cap(2);
        
        imassString = isotopeRegExp.cap(1);
        ...
        if (isotopeRegExp.cap(3).isEmpty()) count = 1;
        else count = isotopeRegExp.cap(3).toInt();
